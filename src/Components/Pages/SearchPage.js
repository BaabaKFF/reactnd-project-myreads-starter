...
class SearchPage extends React.Component {
  ...
  submitSearch() {
    if(this.state.query === '' || this.state.query === undefined) { // This checks if the inputted query is empty or define
      return this.setState({ results: [] });
    }
    BooksAPI.search(this.state.query.trim()).then(res => { 
      //console.log(res);
      if(res.error) { // if the query is empty, this empties the results list 
        return this.setState({ results: [] });
      }
      else { //if the query is defined and found, the results list displays the desired query
        res.forEach(b => {
        ...
        <div className="search-books">
          <div className="search-books-bar">
            ...
            <div className="search-books-input-wrapper">
              <input type="text" placeholder="Search by title or author" value={this.state.query} /*Passes in the new event query event */
